...
class cls_store_designs_accessories extends cls_renderer {
    ...
    function extraHeaders() {
        ...
        <script type="text/javascript">
            ...
            $(function () {
                ...
                $("input:checkbox[name=brand]").click(function () {
                    ...
                    if (len == 1) {
                        var brand = array[0];
                        window.location.href = "store/designs/accessories/brand=" + brand + "/ctg=<?php echo $this->ctg; ?>";
                    } else {
                        window.location.href = "store/designs/accessories/ctg=<?php echo $this->ctg; ?>";
                    }
                });
                //--><!]]>
                //sidebar
        ...
        //            alert(num);
            ...
            function setMRP(dropdown)
            {
                ...
                window.location.href = "store/designs/accessories/brand=<?php echo $this->brand; ?>/ctg=<?php echo $this->ctg; ?>/mrp=" + value;
            }


            function setCategory(dropdown)
            {
                var idx = dropdown.selectedIndex;
                var value = dropdown.options[idx].value;
        //                alert(value);
                if (value != "0") {
                    window.location.href = "store/designs/accessories/ctg=" + value;
                }
            }
        ...
        //        console.log(mrp);
                ...
                window.location.href = "store/designs/accessories/brand=<?php echo $this->brand; ?>/ctg=<?php echo $this->ctg; ?>/size=" + value + mrpclause;
            }

            function search()
            {
                var des_code = document.getElementById("srch").value;
                var ajaxUrl = "ajax/getDesign_accessories.php?ctg_id=<?php echo $this->ctg; ?>"
...
//                            </script>";
            ...
            if ($this->brand != "") {
                if ($this->brand == "z")
                    $brandquery = " and i.brand_id='6' ";
                else
                    $brandquery = " and i.brand_id='5' ";
            } else {
                $brandquery = "";
            }
            ...
            <div class="grid_12">
                 <label>Select Accesories : </label>
            <!--<select  name ="setotherccat" id = "setotherccat" style="width:160px" onchange="setMRP(this);">-->
                         
                 <select name="category"   single style="width:160px;" onchange="setCategory(this);">
                                     <option value=0 selected="selected">Select Category</option>
                                    <?php
                                    $form_value = $this->getFieldValue('category');
                                   // $objs = $db->fetchObjectArray("select * from it_categories where id in(11,33,9,32)  order by name");
                                    $objs = $db->fetchObjectArray("select * from it_categories where setaccesories=1 and active=1 order by name");
                                    foreach ($objs as $obj) {
                                        $selected = "";
                                        if ($obj->id == $this->ctg) {
                                            $selected = "selected";
                                    ...
                                    <option value="<?php echo $obj->id; ?>" <?php echo $selected; ?>><?php echo ucwords(strtolower($obj->name)); ?></option>
                    ...
                    <fieldset class="login">
                        ...
                        <?php if ($this->ctg != 14) { /* Skip Tie's */ ?>
                            ...
                            <?php foreach ($allbrands as $brand) { ?>
                                <input type="checkbox" name="brand" id="brand" value="<?php if ($brand->brand == "Linon") echo "l";
                    else echo "z"; ?>" <?php if ($this->brand == "") echo "checked";
                    else if ($this->brand == "l" && $brand->brand == "Linon") echo "checked";
                    else if ($this->brand == "z" && $brand->brand == "Zinon") echo "checked";
...
//            }
            ...
            foreach ($allDesigns as $design) {
                ...
                <div class="clear"></div>
                <div class="box">
                    <h2 class="expand">Design No: <?php echo $design->design_no . " | Brand: $design->brandname" . " | Material: $design->material" . " | Fabric Type: $design->fabric";
                                if ($design->prodtype) {
                                    echo " [ Production Type: " . $design->prodtype . " ]";
                                } echo " [ MRP: " . $design->MRP . " ] [ Total Qty: $design->tot_qty ]"; ?></h2>
                    <div class="collapse" id="<?php echo $divid; ?>">
                        <form name="order_<?php echo $row_no; ?>" method="post" action="" onsubmit="addToCart(this);
                                return false;">
