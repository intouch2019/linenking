    ...
    function extraHeaders() {
        ...
        <script type="text/javascript">
            <!--//--><![CDATA[//><!--
                ...
                $(function () {
                $("a[rel^='prettyPhoto']").prettyPhoto({animation_speed: 'fast', slideshow: 3000, hideflash: t    rue});
                });
            
                // When the document is re    ady, initialize the link so
                // that when it is cli    cked, the printable area of the
                // page will print.
           ...
           $(".printable").print    ();
            ...
            <script langauge="text/j        avascript">
            function backtoInpic        (pickg            roup)
            {
                var r = confirm("Are you sure this order change back to In-P            icking ?");
                if             (r == true                )
                {
                    //alert(                pickgroup);
                                     $.ajax({
                        ty                    pe: "POST",
                        dataTy                    pe: "json",
                        url: "ajax/backToinpi                    cking.php",
                        data: "pid=" +                    pickgroup,
                        success: function (r                    eturn_data)
                                      {
                            if (return_data.                        error == 0)
                                              {
                                window.location.href = "admin/order                        s/packing";
                                          }
                            if (return_data.                        error == 1)
                                              {
                                alert(return_dat                        a.message);
                                      }
                                  }
...
//	$order = $db->fetchObject("select p.*, o.status, max(o.active_time) as active_time, c.store_name from it_ck_orders o, it_ck_pickgroup p, it_codes c where o.store_id = c.id and o.pickgroup = p.id and o.status = ".OrderStatus::Picking." and p.id=$this->pickgroup_id group by o.pickgroup");
        ...
        <?php if ($this->currStore->usertype == UserType::Admin || $this->currStore->usertype == UserType::Dispatcher) {
            if ($order->status == OrderStatus::Picking_Complete) { ?>  <tr><td colspan="4"></td><td  align="centere"><input type="button" onclick="backtoInpic('<?php echo $this->pickgroup_id; ?>')" value="Change back to In-Picking"/></td></tr><?php }
        } ?>
                                        </table>
            ...
            <?php if ($order->status == OrderStatus::Picking || $order->status == OrderStatus::Picking_Complete) { ?>
                                                <button onclick='window.location = "formpost/orderRevert.php?pid=<?php echo $this->pickgroup_id; ?>"'>Change back to Active</button>
...
//           print "SELECT d.lineno, d.rackno, d.image,d.design_no,o.remarks, d.ctg_id, o.order_qty as total_qty from it_ck_designs d, it_ck_orderitems o, it_items i where o.item_id = i.id and i.ctg_id=21 and o.design_no=d.design_no and o.order_id in ($order->order_ids) order by d.design_no, o.order_id\n";
        ...
        foreach ($allDesigns as $design) {
            ...
            <?php if ($design->remarks) { ?><label style="font-weight: bold; margin-left:15px;">Additional Information : </label><?php print $design->remarks;
                            ...
                            <?php
                            $printHtml .= '<div style="clear:both;font-weight:bold;">' . $design->design_no;
                            if ($design->total_qty) {
                                $printHtml .= ' | Qty : ' . $design->total_qty;
                            }
                            if ($design->remarks) {
                                $printHtml .= '| Requirements :' . $design->remarks;
                            }
                            if ($design->lineno && $design->rackno) {
                                $printHtml .= '| Line: ' . $design->lineno . ', Rack:' . $design->rackno . '</div><br>';
                            } else
                                $printHtml .= '</div><br>'
                                ?>
                                </div><!-- end class="block" -->     
        ...
        <?php
        } if ($order->status == OrderStatus::Shipped && $invids != "") {
            $itemcodelist = substr($itemcodelist, 0, -1);
            if ($itemcodelist != "") {
                $subq = "and item_code not in ($itemcodelist)";
            } else {
                $subq = "";
            }
            ...
            if (count($otheritems) > 0) {
                ?></hr></br><h3>Additional Products</h3><?php
                            }
                            foreach ($otheritems as $o) {
                                ...
                                if (isset($newitem)) {
                                    ...
                                    ?>
                                    <div class="box">
                                        <h2>CATEGORY: <?php echo $newitem->ctg_name . " | DESIGN NO: $newitem->design_no | ";
                    echo "[ MRP: " . $newitem->MRP . " ]"; ?></h2>
                                        <div class="block" id="<?php echo $divid; ?>">
                                            ...
                                            <div class="block grid_10" style="margin-top:30px;">
                                                ...
                                                <label style="font-weight: bold; margin-left:10px;">Quantity : </label> <?php print $o->quantity;
                    $totextra += $o->quantity; ?>
                                                </br>
                                            </div>
                                        </div><!-- end class="block" -->     
        ...
        <?php if ($order->status == OrderStatus::Shipped && $invids != "") {
            if ($totextra > 0) {
                echo "<h4>Total Extra Shipped : $totextra</h4>";
                $totdel += $totextra;
            } ?>
            <?php $diff = $totord - $totdel;
            if ($diff > 0) echo "<h4>Total Dropped : $diff</h4>";
            else {
                $diff = $diff * -1;
                echo "<h4>Total Additional : $diff</h4>";
            }
        } ?>
                        </div>
                    </div>
