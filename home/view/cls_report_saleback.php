...
class cls_report_saleback extends cls_renderer{
        var $currUser;
        var $userid;
        var $dtrange;
        var $params;
      ...
      //  var $category; 
       var $style; 
       var $view;
      // var $prize; var $fabric; var $material;
       // var $prodtype; var $manuf; 
...
<div class="grid_10">
 ...
 ?>
	...
	<legend>Generate Sale Back Report</legend>
...
<!--                                          <option value="barcode">Barcode</option>
                                        ...
                                        <option value="taxableamt">Taxable Amount</option>
                                          <option value="discval">Discount Value</option>-->


                                          <option value="mrpval">Amount</option>
                                          <option value="total_quantity">Quantity</option>
                                               
                                          <option value="date">Date</option>
<!--                                    <option value="view" ><a href="">Show details</a></option>-->
                                       
<!--                                      <option value="billtype" selected>Invoice Type</option>
...
<!--                                        <option value="totalvalue">Total Value</option>-->
	...
	if (count($dtarr) == 1) {
        ...
        if ($this->gen!=1) {
            ...
            $newfname = "Bill_Saleback_".$sdate."_".$edate.".csv";           
            ...
            for ($x=1;$x<23;$x++) {
                foreach ($this->fields as $field => $seq) {
                    if ($seq==$x) {
                     ...
                     //   if ($field=="billtype") {$tableheaders.="Invoice Type.:";  $queryfields .= "o.invoice_type,"; $group_by[] = "o.invoice_type"; $total_td .= "<td></td>";}
                     ...
                     //   if ($field=="barcode") {$tableheaders.="Barcode:"; $queryfields .= "oi.item_code,";$group_by[] = "oi.item_code"; $total_td .= "<td></td>";}
                      //  
                        
                        
                  //    if ($field=="taxableamt") {$tableheaders.="Taxable Ammount:"; $queryfields .= "round(oi.taxable_value,2) as taxable,";}
                        if ($field=="mrpval") {$tableheaders.="Invoice Ammount:"; $queryfields .= "i.invoice_amt as invamt,";}
                       //if ($field=="view") {$tableheaders.="Show Details:";  }
                       
                      
                       
                    //   if ($field=="tax") {$tableheaders.="Tax on Bill Amt:"; $queryfields .= "o.tax,";}
          //      if ($field=="month") {$tableheaders.="Month:"; $queryfields .= " CONCAT(monthname(o.invoice_dt),'-',year(o.invoice_dt)) as month , ";$group_by[] = "month"; $total_td .= "<td></td>";}
...
//            }
      ...
      $query.=" from it_codes c,it_saleback_invoices i where $storeClause $dQuery and c.id=i.store_id and i.invoice_type in(6,7) ".$gClause;
...
?>
        <br /><div id="dwnloadbtn" style='margin-left:40px; padding-left:15px; height:24px;width:130px;border: solid gray 1px;background:#F5F5F5;padding-top:4px;'>
            <a href='<?php echo "tmp/storesaleback.php?output=$newfname" ;?>' title='Export table to CSV'><img src="images/excel.png" width='20' hspace='3' style='margin-bottom:-6px;' /> Export To Excel</a>
        </div><br />
...
<?php 
   $totqty=0;$totsp=0;
    if (isset($result)) {
        ...
        $fp = fopen('tmp/StoreSalesBack.csv', 'w');
           
        if($write_htm){
         $fp2 = fopen ('tmp/storesalesBack.htm', 'w');
        } 
        if ($fp) {
               ...
               if($write_htm){ 
                ...
                if($write_htm){
                   ...
                   //print_r($reportrows);
                    ...
                    //print $invid;
                  fwrite($fp2,"<td><a href='lk/sbinvoice/id=$invid'  style='color:#cc0000'>Show Details</a></td>");
                 fwrite($fp2,"</tr>");
                }
...
//                $totTotalValue=$totAmt;
        //    }
            ...
            fclose ($fp); 
            if($write_htm){
                $table = file_get_contents("tmp/storesalesBack.htm");
                echo $table;
            }
        } else {
      ...
      // var storeid = $('#store').val();      
       ...
       if (storeid!="" && storeid != null) {
           if (reporttype=="itemwise") {
             ...
             window.location.href="report/saleback/str="+storeid+append+aclause;
                // window.location.href="ck/sbinvoice/str="+storeid+append+aclause;
             
           } else {
                window.location.href="report/saleback/str="+storeid+"/gen=1"+aclause;
           }
           $('#dwnloadbtn').show();
       } else {
