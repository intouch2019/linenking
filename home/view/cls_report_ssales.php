...
class cls_report_ssales extends cls_renderer{
        ...
        var $dtrange;
        var $params;
        var $date; var $day; var $store; var $billno; var $transaction;
...
//                if (isset($params['category'])) { $this->fields['category']=$params['category']; $this->category = $params['category']; } else $this->fields['category']="0";
                ...
                if (isset($params['custphone'])){ $this->fields['custphone'] =$params['custphone']; $this->cust = $params['custphone']; }else{ $this->fields['custphone'] = "0"; }
                ...
                if (isset($params['revisedmrp'])) { $this->fields['revisedmrp'] = $params['revisedmrp']; $this->coupon = $params['revisedmrp']; } else $this->fields['revisedmrp'] = "";
...
<!--                                        <option value="itemvalue">Sold Price</option>-->
                                        ...
                                        <option value="nnetsale" selected>Bill MRP</option>
                                        <option value="monthlyrent" selected>Monthly Rent</option>
                                    </select>
                                </label></td>
            ...
            for ($x=1;$x<23;$x++) {
                foreach ($this->fields as $field => $seq) {
                    if ($seq==$x) {
                        ...
                        if ($field=="monthlyrent") {$tableheaders.="Monthly Rent:"; $queryfields .= "c.monthlyrent,";$group_by[] = "o.store_id"; $total_td .= "<td></td>";}
                        ...
                        if ($field=="barcode") {$tableheaders.="Barcode:"; $queryfields .= "oi.barcode,";$group_by[] = "oi.barcode"; $group_by[] = "oi.id"; $total_td .= "<td></td>";}
                        ...
                        if ($field=="ticketdiscountval") {$tableheaders.="Ticket Discount Value:"; $queryfields .= "sum(oi.discount_val) as totdiscv,";}
                        ...
                        if ($field=="manuf") {$tableheaders.="Mfg By:"; $queryfields .= "i.mfg_id as manuf,";$group_by[] = "i.mfg_id"; $total_td .= "<td></td>";}                       
...
//                                        $total_td .= "<td ></td>";
     ...
     ROUND(
        SUM(
            CASE WHEN o.tickettype IN (0, 6)
                 THEN ( i.MRP -
                     ( IFNULL(oi.discount_val, 0)
                       + IFNULL(
                            CASE WHEN o.discount_pct IS NOT NULL
            ...
            END
                                    ...
                                    if ($field == "nnetsale") { 
                                        $tableheaders .= "Bill MRP:";
                                        $queryfields .= "CASE 
    WHEN o.tickettype <= 0 THEN (
        SELECT SUM(bb.mrp) 
        FROM it_items bb
        JOIN it_order_items ooi ON bb.id = ooi.item_id
        WHERE ooi.order_id = o.id
    )
    ELSE 0
END AS net_total,";
                                        $total_td .= "<td ></td>";
...
$queryfields .= "sum(oi.discount_val) as _internal_totdiscv,";
$queryfields .= "op.loyalty,";
$queryfields .= "op.loyaltotal,";
$queryfields .= "op.isexchange,";
...
//                    $storeClause = " c.usertype = ".UserType::Dealer ;
                    $storeClause = " o.store_id in ( select id from it_codes where usertype = ".UserType::Dealer."   ) ";
                    $this->storeidreport = "select id from it_codes where usertype = ".UserType::Dealer;
                }
//                $storeClause = " c.usertype = ".UserType::Dealer ;
            }else{              
 ...
 JOIN it_items i ON i.id = oi.item_id
 JOIN (
      SELECT iop.order_id,
            MAX(CASE WHEN iop.payment_name = 'Loyalty' THEN 1 ELSE 0 END) AS loyalty,
            MAX(CASE WHEN iop.payment_name = 'Loyalty' THEN iop.amount ELSE 0 END) AS loyaltotal,            
            MAX(CASE WHEN iop.payment_name = 'paperin' THEN 1 ELSE 0 END) AS isexchange
      FROM it_order_payments iop
      JOIN it_orders io ON iop.order_id = io.id
      WHERE io.store_id IN ( $this->storeidreport ) " . // Corrected: use io.store_id and $this->storeidreport
            (count($dtarr) == 1 ? " AND io.bill_datetime >= '$sdate 00:00:00' AND io.bill_datetime <= '$sdate 23:59:59' " : "") . // Corrected: use io.bill_datetime and $sdate
            (count($dtarr) == 2 ? " AND io.bill_datetime >= '$sdate 00:00:00' AND io.bill_datetime <= '$edate 23:59:59' " : "") . // Corrected: use io.bill_datetime, $sdate, $edate
      " GROUP BY iop.order_id
 ) op ON op.order_id = o.id
 JOIN it_codes c ON o.store_id = c.id
 WHERE $storeClause $dQuery $gClause";
//            print $query;exit(); //and c.id in ( $storeClause)
	    //error_log("1:$query\n",3, "../ajax/tmp.txt");
...
//                $storeClause = " c.usertype = ".UserType::Dealer;
 ...
 JOIN (
      SELECT iop.order_id,
            MAX(CASE WHEN iop.payment_name = 'Loyalty' THEN 1 ELSE 0 END) AS loyalty,
            SUM(CASE WHEN iop.payment_name = 'Loyalty' THEN iop.amount ELSE 0 END) AS loyaltotal,
            MAX(CASE WHEN iop.payment_name = 'paperin' THEN 1 ELSE 0 END) AS isexchange
      FROM it_order_payments iop
      JOIN it_orders io ON iop.order_id = io.id
      WHERE io.store_id IN ( $this->storeidreport ) " . // Corrected: use io.store_id and $this->storeidreport
...
//               print_r($reportrows);echo '<br>';//exit();
                foreach ($reportrows as $field => $value) {
                     if ($field == "_internal_totdiscv") continue;
                     if ($field == "payment") continue;
                     if ($field == "loyaltotal") continue;
                     if ($field == "loyalty") continue;                     
                     if ($field == "isexchange") continue;
                     
                   if ($field=="tax") {
                       $value = sprintf('%.2f',$value);
                   } else if ($field == "tickettype") {
    $discount = isset($reportrows->_internal_totdiscv) ? (float)$reportrows->_internal_totdiscv : 0;
    $loyaldiscount = isset($reportrows->loyaltotal) ? (int)$reportrows->loyaltotal : 0;
    $hasLoyalty = isset($reportrows->loyalty) && $reportrows->loyalty == 1;
    $isExchange = isset($reportrows->isexchange) && $reportrows->isexchange == 1;
    $quantity = isset($reportrows->quantity) ? (int)$reportrows->quantity : 0;
    $totalsale = $reportrows->totalvalue;
    if(!empty($hasLoyalty) || $hasLoyalty!=""){
      $isLoyalDiscount  = 1;
    }else{
      $isLoyalDiscount=0;
    }
//     echo '<br>'; print_r($billno."<-->".$isLoyalDiscount."<-->".$discount."<-->".$totalsale);
    if ($value == '3') {
        $value = 'Cancelled';
    } else if($value == "1"){
         $value = 'Credit Note';
    }
    else if ($isLoyalDiscount==1 &&  $loyaldiscount == 500) {
        $value = 'Hurdle 1';
    } else if ($isLoyalDiscount==1 && $loyaldiscount==1000) {
        $value = 'Hurdle 2';
    }
   
     else if ($isLoyalDiscount==0 && $discount > 0 &&  $totalsale > 3997 && $totalsale< 6999) {
        $value = 'Hurdle 1';
    } else if ($isLoyalDiscount==0 && $discount > 0 &&  $totalsale > 3997==0 && $discount > 0 &&  $totalsale > 6999 ) {
        $value = 'Hurdle 2';
    }
    
    else if ($isLoyalDiscount==0 && $discount > 0 &&  $totalsale < 3997 ) {
        $value = 'Discount';
