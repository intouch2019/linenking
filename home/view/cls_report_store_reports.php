...
class cls_report_store_reports extends cls_renderer{
        var $currUser;
        var $userid;
        var $dtrange;
        var $params;
...
//                if (isset($params['loyalty'])){ $this->fields['loyalty'] =$params['loyalty']; $this->loyalty = $params['loyalty']; }else{ $this->fields['loyalty'] = "0"; }
                ...
                if (isset($params['city'])) { $this->fields['city']=$params['city']; $this->city = $params['city']; } else $this->fields['city']="0";
                if (isset($params['location'])) { $this->fields['location']=$params['location']; $this->location = $params['location']; } else $this->fields['location']="0";
...
//    if($objs){ foreach($objs as $obj){ $categories[$obj->id] = $obj->name;}}
    ...
    $query = "select id,name,it_hsncode from it_categories ";
    $objs= $db->fetchObjectArray($query);
    if($objs){ foreach($objs as $obj){ $categories[$obj->id] = $obj->name; $hsncodeqry[$obj->id] = $obj->it_hsncode;}}
            ...
            for ($x=1;$x<23;$x++) {
                foreach ($this->fields as $field => $seq) {
                    if ($seq==$x) {
                        ...
                        if ($field=="store") {$tableheaders.="Store Name:"; $queryfields .= "o.store_name,";$group_by[] = "o.store_id"; $total_td .= "<td></td>";}
...
//                        if ($field=="itemqty") {$tableheaders.="Quantity:"; $queryfields .= "sum(oi.quantity) as quantity,";}
                        ...
                        if ($field=="barcode") {$tableheaders.="Barcode:"; $queryfields .= "oi.barcode,";$group_by[] = "oi.barcode"; $total_td .= "<td></td>";}
                        if ($field=="linediscountper") {$tableheaders.="Line Discount %:"; $queryfields .= "oi.discount_pct as itmdiscp,"; $group_by[] = "oi.discount_pct "; $total_td .= "<td></td>";}
                        //if ($field=="linediscountval") {$tableheaders.="Line Discount Value:"; $queryfields .= "sum(oi.discount_val) as itmdiscv,";}
                        if ($field=="linediscountval") {$tableheaders.="Line Discount Value:"; $queryfields .= "sum(case when (o.tickettype = 0 ) then oi.discount_val else 0 end) as itmdiscv,";}
                        if ($field=="ticketdiscountper") {$tableheaders.="Ticket Discount %:"; $queryfields .= "o.discount_pct as totdiscp,"; $group_by[] = "o.discount_pct"; $total_td .= "<td></td>";}
                        ...
                        if ($field=="brand") {$tableheaders.="Brand:"; $queryfields .= "oi.brand_id as brand,"; $group_by[] = "oi.brand_id"; $total_td .= "<td></td>";}
                        if ($field=="style") {$tableheaders.="Style:"; $queryfields .= "oi.style as style,"; $group_by[] = "oi.style"; $total_td .= "<td></td>";}
                        if ($field=="size") {$tableheaders.="Size:"; $queryfields .= "oi.size_id as size,"; $group_by[] = "oi.size_id"; $total_td .= "<td></td>";}
                        if ($field=="fabric") {$tableheaders.="Fabric:"; $queryfields .= "oi.fabric_type_id as fabric,";$group_by[] = "oi.fabric_type_id"; $total_td .= "<td></td>";}
                        if ($field=="material") {$tableheaders.="Material:"; $queryfields .= "oi.material_id as material,"; $group_by[] = "oi.material_id"; $total_td .= "<td></td>";}
                        if ($field=="prodtype") {$tableheaders.="Production Type:"; $queryfields .= "oi.prod_type_id as prodtype,"; $group_by[] = "oi.prod_type_id"; $total_td .= "<td></td>";}
                        if ($field=="manuf") {$tableheaders.="Mfg By:"; $queryfields .= "oi.mfg_id as manuf,";$group_by[] = "oi.mfg_id"; $total_td .= "<td></td>";}                       
                        if ($field=="month") {$tableheaders.="Month:"; $queryfields .= " o.month as month , ";$group_by[] = "o.month"; $total_td .= "<td></td>";}
                        ...
                        if ($field=="custphone") {$tableheaders.="Customer Phone:"; $queryfields .= "o.cust_phone as customerphone , ";$group_by[] = "customerphone"; $total_td .= "<td></td>";}
...
//                        if ($field=="loyalty") {$tableheaders.="Loyalty Points:"; $queryfields .= " sum(o.use_lpoints) as loyalty , "; $total_td .= "<td></td>";}
                        ...
                        if ($field=="area") {$tableheaders.="Area:"; $queryfields .= "o.Area,";$group_by[] = "o.Area"; $total_td .= "<td></td>";}
                        if ($field=="city") {$tableheaders.="City:"; $queryfields .= "o.city,";$group_by[] = "o.city"; $total_td .= "<td></td>";}
                        if ($field=="location") {$tableheaders.="Location:"; $queryfields .= "o.Location,";$group_by[] = "o.Location"; $total_td .= "<td></td>";}
                        if ($field=="state") {$tableheaders.="state:"; $queryfields .= "o.state,";$group_by[] = "o.state"; $total_td .= "<td></td>";}
                        if ($field=="region") {$tableheaders.="region:"; $queryfields .= "o.region,";$group_by[] = "o.region"; $total_td .= "<td></td>";}
                        if ($field=="status") {$tableheaders.="Status:"; $queryfields .= "o.status,";$group_by[] = "o.status"; $total_td .= "<td></td>";}
...
//            }
            $tableheaders.="Quantity:"; $queryfields .= "sum(case when (o.tickettype in (0,1,6) ) then oi.quantity else 0 end) as quantity,";
            ...
            if($this->storeidreport == "-1"){               
                 if($this->currUser->usertype == UserType::BHMAcountant ) {
                $storeClause = " o.usertype = ".UserType::Dealer." and  (is_bhmtallyxml=1 or store_type=3)" ;
                }else{
                    $storeClause = " o.usertype = ".UserType::Dealer ;
                }
...
//                $storeClause = " c.usertype = ".UserType::Dealer ;
           ...
           $query2 = "select o.bill_datetime ,o.bill_no,o.tickettype,o.quantity,o.amount,o.tax,o.discount_val,o.discount_pct,o.voucher_amt,o.store_name,o.Area,o.city,o.Location, o.state,o. region,o.status from it_salesreports o,it_salereport_items oi  where $storeClause  and  oi.order_id=o.id $dQuery  group by o.store_id, o.bill_no order by bill_datetime;";
            // echo $query2;
     ...
     var ret = dateRange();
      ...
      //alert("1: "+storeid);
        ...
        if(storeid=='-1'){
         ...
         var ajaxUrl= "ajax/sales_rep.php?sid="+storeid+"&date="+dttrange;   
             //  alert(storeid);
                        $.ajax({
                             url: ajaxUrl,
                              Type: "GET" ,
                          //  data: { var1 :$storeClause },
                            success: function( data ) {
                              //  alert( data );
      ...
      // var storeid = $('#store').val();      
       ...
       if (storeid!="" && storeid != null) {
           if (reporttype=="itemwise") {
                ...
                window.location.href="report/store/reports/str="+storeid+append+aclause;
           } else {
                window.location.href="report/store/reports/str="+storeid+"/gen=1"+aclause;
           }
           $('#dwnloadbtn').show();
       } else {
