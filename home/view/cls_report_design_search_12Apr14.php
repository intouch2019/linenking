...
//    }
	...
	$menuitem = "gridDesignSearch";
        ...
        $usertype = $this->currStore->usertype;
        $db = new DBConn();
        $allDesigns = array();
        $storeClause="";$pClause="";
        ?>
...
//               if(isset($this->storeidreport) && trim($this->storeidreport) != ""){     
              ...
              // $query = " select c.barcode ,c.quantity,i.style_id,i.size_id,i.ctg_id,i.design_no,i.MRP,ctg.name as category,d.image,d.lineno,d.rackno,d.active from it_current_stock c , it_items i, it_ck_designs d, it_categories ctg  where c.barcode in (select barcode from it_items where design_no = $this->param_design_no ) and c.barcode = i.barcode and i.ctg_id =d.ctg_id and i.ctg_id = ctg.id and i.design_no = d.design_no  $pClause $storeClause group by i.ctg_id";
                $query = " select c.barcode, sum(c.quantity) as currqty,i.barcode,i.ctg_id,i.MRP,i.style_id,i.size_id,i.prod_type_id ,i.design_no ,d.active,d.image,d.lineno,d.rackno from it_current_stock c, it_items i , it_ck_designs d  where c.barcode = i.barcode $storeClause and i.design_no = d.design_no and i.ctg_id = d.ctg_id and  i.design_no = $this->param_design_no $pClause  group by i.ctg_id ";
                echo $query;
                print "\n ST ID: ".$this->storeidreport;
...
//               }
                ...
                //design_no 4r testing => 1532,8753
            }
            ?>
...
if ($this->param_design_no != "" && count($allDesigns) == 0) { ?>
	<p class="error">No records found matching "<?php echo $this->param_design_no; ?>"</p>
<?php } ?>
        </fieldset>
    </div>
...
//                echo "select name from it_prod_types where id in ($this->ptype)";
                ...
                $ctgobj = $db->fetchObject("select name from it_categories  where ctg_id = $design->ctg_id ");
                if($ctgobj){ $category = $ctgobj->name;}else{ $category = "";}
                ?>
    <div class="clear"></div>
    <div class="box">
        <h2 class="expand"><?php echo $category; ?> [<?php echo $state; ?>] Design No: <?php echo $design->design_no." [ MRP: ".$design->MRP." ]"; ?>  [Production Type: <?php if($this->ptype == "0"){ echo "ALL PRODUCTION TYPE";}else{ echo $prod_name; } ?>]</h2> <!--[ Line No: <?php // echo $design->lineno; ?> ] [ Rack No: <?php // echo $design->rackno; ?> ]-->
