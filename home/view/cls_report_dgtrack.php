...
class cls_report_dgtrack extends cls_renderer {
...
//		parent::__construct(array(UserType::Admin, UserType::CKAdmin, UserType::Manager));
        ...
        if (isset($params['is_saleback'])) {
            $this->fields['is_saleback'] = $params['is_saleback'];
            $this->is_saleback = $params['is_saleback'];
        } else
            $this->fields['is_saleback'] = "0"; //salebackdate
        if (isset($params['salebackdate'])) {
            $this->fields['salebackdate'] = $params['salebackdate'];
            $this->salebackdate = $params['salebackdate'];
        } else
            $this->fields['salebackdate'] = "0";
...
//            $menuitem = "bnewbatch";
        ...
        <div class="grid_10">
            ...
            $query = "select id,name,it_hsncode from it_categories ";
...
//    $query = "select id,name from it_categories ";
//    $objs= $db->fetchObjectArray($query);
//    if($objs){ foreach($objs as $obj){ $hsncodeqry[$obj->id] = $obj->it_hsncode;}}
            //create brands array
            $query = "select id,name from it_brands ";
            $objs = $db->fetchObjectArray($query);
            if ($objs) {
                foreach ($objs as $obj) {
            ...
            //create style array
            $query = "select id,name from it_styles ";
            $objs = $db->fetchObjectArray($query);
            if ($objs) {
                foreach ($objs as $obj) {
            ...
            //create mfg by array
            $query = "select id,name from it_mfg_by ";
            $objs = $db->fetchObjectArray($query);
            if ($objs) {
                foreach ($objs as $obj) {
            ...
            $query = "select id,name from it_prod_types ";
            $objs = $db->fetchObjectArray($query);
            if ($objs) {
                foreach ($objs as $obj) {
            ...
            $query = "select id,name from it_fabric_types ";
            $objs = $db->fetchObjectArray($query);
            if ($objs) {
                foreach ($objs as $obj) {
            ...
            $query = "select id,name from it_materials ";
            $objs = $db->fetchObjectArray($query);
            if ($objs) {
                foreach ($objs as $obj) {
            ...
            <div class="box" style="clear:both;">
                <fieldset class="login">
                    <legend>Generate Defective Garment Report</legend>
                    ...
                    <form action="" method="" onsubmit="reloadreport();
                        ...
                        <div class="grid_12">
                            ...
                            if ($this->currUser->usertype != UserType::Dealer) {
                                //($this);
                                if ($this->currUser->usertype == UserType::Manager && $this->currUser->roles == RollType::Sales) {
                                    
                                }
//                                $idd = $this->currUser->region_assignid;
//                                $fidd = str_replace(",", " or region_id=", $idd);
                                ?>           
...
//                                                        $rmvcoln=substr($st, 0, -1);
                                    //print_r("select id,store_name from it_codes where usertype=4 and id in ($rmvcoln) order by store_name");
                                    ?>
                                    ...
                                    <select name="store" id="store" data-placeholder="Choose Store" class="chzn-select" multiple style="width:100%;">
                                        ...
                                        <?php if ($this->currUser->usertype == UserType::Admin || $this->currUser->roles == RollType::Accounts || $this->currUser->roles == RollType::Management || $this->currUser->roles == RollType::IT || $this->currUser->roles == RollType::Accounts || $this->currUser->roles == RollType::Marketing) { ?>
                                                <!--                                            <option value="-1" <?php echo $defaultSel; ?>>All Stores</option> -->

                                        <?php } ?>
                                        <?php if ($this->currUser->usertype == UserType::Manager && $this->currUser->roles == RollType::Sales) {
                                            ?>
                                        ...
                                        $objs = array();
                                        //$usrid=$this->currUser->id;
                                        // $objs = $db->fetchObjectArray("select id,store_name from it_codes where usertype=4 and executive_assign =$usrid order by store_name");
                                        // print_r($objs);
                                        //print_r("select id,store_name from it_codes where usertype=4 and executive_assign =$usrid order by store_name");
//                                        if ($this->currUser->usertype == UserType::BHMAcountant) {
//                                            $objs = $db->fetchObjectArray("select id,store_name from it_codes where usertype=4 and  (is_bhmtallyxml=1 or store_type=3) order by store_name");
//                                        } else if ($this->currUser->usertype == UserType::Admin || $this->currUser->roles == RollType::Management || $this->currUser->roles == RollType::IT || $this->currUser->roles == RollType::Accounts || $this->currUser->roles == RollType::Marketing) {
//                                            $objs = $db->fetchObjectArray("select id,store_name from it_codes where usertype=4 order by store_name");
//                                        } else if ($this->currUser->usertype == UserType::Manager && $this->currUser->roles == RollType::Sales) {
                                        $usrid = $this->currUser->id;
...
//	if (isset($this->storeidreport) && $obj->id==$this->storeidreport) { $selected = "selected"; }
                                    ...
                                    <tr>
                                        <td rowspan="3" colspan="2" align="right"><label>
                                                <select name="selectLeft" size="10" width="100%" style="width:200px;" id="selectLeft"> 
                                                    ...
                                                    <option value="is_saleback">Saleback Status</option>
                                                    <option value="salebackdate">Saleback Date</option>
                                                    ...
                                                    <option value="date" selected>DGreturn Date</option>
                                                    ...
                                                    <option value="creditvoucherused" selected>Credit voucher Used</option>
                                                    <option value="salesmancode" selected>Salesman ID</option>
                                                    <!--                                        <option value="itemvalue">Sold Price</option>-->
                                                    <option value="month">Month</option>
                                                    <?php if ($this->currUser->usertype == UserType::Admin || $this->currUser->usertype == UserType::CKAdmin || $this->currUser->usertype == UserType::Manager || $this->currUser->usertype == UserType::Dealer) { ?>
                        ...
                        <div class="grid_12" id="generalselection" style="display:none;">
                            <div class="grid_12" >
                                ...
                                (Date/Bill no/Bill quantity/Bill amount/Tax/Bill discount value/Bill discount %//Voucher/Store name/Saleback Status/Saleback Date/Facade/Carpet Area/Area/City/Location/State/Region/Status)
                            </div>
                        </div>
                        ...
                        for ($x = 1; $x < 23; $x++) {
                            foreach ($this->fields as $field => $seq) {
                                if ($seq == $x) {
                                    ...
                                    if ($field == "is_saleback") {
                                        $tableheaders .= "Saleback Status:";
                                        $queryfields .= "o.is_saleback,";
                                        //$group_by[] = "o.is_saleback";
                                        $total_td .= "<td></td>";
                                    }//salebackdate
                                    if ($field == "salebackdate") {
                                        $tableheaders .= "Saleback Date:";
                                        $queryfields .= "o.saleback_time,";
                                        //$group_by[] = "o.saleback_time";
                                        $total_td .= "<td></td>";
                                    }
                                    if ($field == "facade") {
...
//                        if ($field=="itemqty") {$tableheaders.="Quantity:"; $queryfields .= "sum(oi.quantity) as quantity,";}
                                    ...
                                    if ($field == "linediscountval") {
                                        ...
                                        $queryfields .= "oi.discount_val as itmdiscv,";
                                    }
                                    ...
                                    if ($field == "state") {
                                        $tableheaders .= "state:";
                                        $queryfields .= "if((select state from states where id=c.state_id) is null,'-', (select state from states where id=c.state_id)) as state,";
                                        $group_by[] = "c.state_id";
                                        $total_td .= "<td></td>";
                                    }
                                    if ($field == "region") {
                                        $tableheaders .= "region:";
                                        $queryfields .= "if((select region from region where id=c.region_id) is null,'-', (select region from region where id=c.region_id)) as region,";
                                        $group_by[] = "c.region_id";
                                        $total_td .= "<td></td>";
                                    }
                                    if ($field == "status") {
                                        $tableheaders .= "Status:";
...
//            }
                        ...
                        $queryfields .= "sum(case when (o.tickettype in (1) ) then (oi.quantity*(-1)) else 0 end) as quantity,";
                        $tableheaders .= "Total Value:";
                        $queryfields .= "sum(case when (o.discount_pct is not NULL) then ((((100-o.discount_pct)/100)*oi.price) * (case when (o.tickettype in (1)) then (oi.quantity) else 0 end )) else oi.price*(case when (o.tickettype in (1)) then (oi.quantity*(-1)) else 0 end ) end) as totalvalue,";
                        ...
                        //$query .= " from it_orders o,it_order_items oi, it_items i, it_codes c where $storeClause $dQuery and oi.order_id=o.id and i.id = oi.item_id and  o.store_id = c.id ".$gClause;
                        $query .= " from it_orders o,it_order_items oi, it_items i, it_codes c where $storeClause $dQuery and oi.order_id=o.id and i.id = oi.item_id and  o.store_id = c.id and o.isdg=1" . $gClause;
...
//                        exit;
                        ...
                        if ($this->storeidreport == "-1") {
                            ...
                            //
//                            $usrid=$this->currUser->id;
//                                        $result = $db->fetchObjectArray("select id,store_name from it_codes where usertype=4 and executive_assign =$usrid order by store_name");
                            foreach ($result as $re) {
                                $a = $re->id;
                                $b .= $a . ",";
                            }
                        ...
                        $tableheaders = "Date:Bill No:Bill Type:Bill Quantity:Bill Amount:Tax:Bill Discount Value:Bill Discount %:Voucher:Store Name:Saleback Status:Saleback Date:Facade:Carpet Area:Area:city:Location:State:Region:Status";
                        ...
                        //$query2 = "select o.bill_datetime ,o.bill_no,o.tickettype,o.quantity,o.amount,o.tax,o.discount_val,o.discount_pct,o.voucher_amt,c.store_name,c.Area,c.city,c.Location,if((select state from states where id=c.state_id) is null,'-', (select state from states where id=c.state_id)) as state,if((select region from region where id=c.region_id) is null,'-', (select region from region where id=c.region_id)) as region,c.status from it_orders o , it_codes c where $storeClause  and  o.store_id = c.id $dQuery group by o.store_id, o.bill_no order by bill_datetime";
                        $query2 = "select o.bill_datetime ,o.bill_no,o.orderinfo,o.voucher_amt,o.tickettype,o.quantity,o.amount,o.tax,o.discount_val,o.discount_pct,o.voucher_amt,c.store_name,c.Area,c.city,c.Location,if((select state from states where id=c.state_id) is null,'-', (select state from states where id=c.state_id)) as state,if((select region from region where id=c.region_id) is null,'-', (select region from region where id=c.region_id)) as region,c.status from it_orders o , it_codes c where $storeClause  and o.isdg=1 and  o.store_id = c.id $dQuery group by o.store_id, o.bill_no,o.voucher_amt order by bill_datetime";
//            echo $query2;
                        //print_r($query2);
...
//                                     echo "??";
       ...
       <script src="js/chosen/chosen.jquery.js" type="text/javascript"></script>
        ...
        //                                                            alert(multiplevalues);
                                                            ...
                                                            window.location.href = "report/dgtrack/str=" + storeid + append + aclause;
                                                        } else {
                                                            window.location.href = "report/dgtrack/str=" + storeid + "/gen=1" + aclause;
                                                        }
                                                        $('#dwnloadbtn').show();
                                                    } else {
