...
class cls_rstore_designs_others extends cls_renderer {
...
//alert(data.error+":"+data.message+":"+data.cartinfo+":"+data.totqt+":"+data.totmrp);
    ...
    public function pageContent() {
        ...
        $menuitem = "r_21"; // category 'others' id is 21
...
//               $query = "select d.* from it_ck_designs d where d.active=1 and d.ctg_id=(select id from it_categories where name = 'Others')";
              $othctg = $db->fetchObject("select * from it_categories where  name = 'Others'");
              $query = "select d.*,i.id as item_id,i.MRP from it_ck_designs d, it_items i where i.is_design_mrp_active = 1 and d.ctg_id = i.ctg_id and d.design_no = i.design_no  and d.ctg_id = $othctg->id order by i.design_no ";
              //print $query;
...
//                    $query = "select order_qty,remarks from it_ck_orderitems where order_id=$cart->id and design_no=$design_no and ctg_id=(select id from it_categories where name = 'Others')";
                    ...
                    $query = "select sum(curr_qty) as qty from it_items i where i.ctg_id = 21 and i.design_no = $design_no ";
                    //print $query;
                ...
                <h2 class="expand">Product : <?php echo $design->design_no;  ?> </h2>
                    ...
                    <form name="order_<?php echo $row_no; ?>" method="post" action="" onsubmit="addToCart(this); return false;">
                        <input type="hidden" name="ctg_id" value="21" />
                        ...
                        <a href="<?php echo $this->designImageUrl($design->image); ?>" rel=""><img class="grid_2" align="left" src="<?php echo $this->designImageUrl($design->image); ?>" width="170" /></a>
                        <div class="block grid_10" style="margin-top:30px;">
                            <div id="status_<?php echo $row_no; ?>"></div>  
                            <label style="font-weight: bold; margin-left:15px;">Quantity : </label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="quantity" style="width:35%;" <?php if ($exist) { print "value='".$exist->qty."'"; }?> readonly></input><br/><br/>                            
