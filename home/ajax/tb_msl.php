...
$sQuery = "
	select SQL_CALC_FOUND_ROWS c.id,c.store_name,c.min_stock_level
	from it_codes c
	$sWhere 
	$sOrder
	$sLimit
...
{       $tot_stk = 0;
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
             if ($aColumns[$i] == 'id') {
              ...
              //appreal_curr_stock
             else if($aColumns[$i] == 'appreal_curr_stock'){
                 $cquery = "select sum(c.quantity * i.MRP) as appreal_curr_stock from it_current_stock c , it_items i where c.store_id = $obj->id  and c.barcode = i.barcode and i.ctg_id not in (53,54) ";
             ...
             //mask_curr_stock 
                 ...
                 $cquery = "select sum(c.quantity * i.MRP) as mask_curr_stock from it_current_stock c , it_items i where c.store_id = $obj->id  and c.barcode = i.barcode and i.ctg_id  in (53,54) ";
                 $cobj = $db->fetchObject($cquery);
           ...
           //total_curr_stock
           ...
           else if($aColumns[$i] == 'appreal_stock_intransit'){
                 $tquery2 = "select sum(i.MRP*oi.quantity) as appreal_stock_intransit from it_sp_invoices o , it_sp_invoice_items oi , it_items i where oi.invoice_id = o.id and o.invoice_type in ( 0 , 6 ) and o.store_id = $obj->id and o.is_procsdForRetail = 0 and oi.barcode = i.barcode and i.ctg_id not in (53,54)";                 
             ...
             //mask_stock_intransit  
                 ...
                 $tquery2 = "select sum(i.MRP*oi.quantity) as mask_stock_intransit from it_sp_invoices o , it_sp_invoice_items oi , it_items i where oi.invoice_id = o.id and o.invoice_type in ( 0 , 6 ) and o.store_id = $obj->id and o.is_procsdForRetail = 0 and oi.barcode = i.barcode and i.ctg_id  in (53,54)";                 
                 $tobj = $db->fetchObject($tquery2);
                 if(isset($tobj) && trim($tobj->mask_stock_intransit)!=""){ $intransit_mask_val = $tobj->mask_stock_intransit ;}
