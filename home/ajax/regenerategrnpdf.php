...
require "lib/grnPDFClass/fpdf.php";

extract($_GET);
$desids = $_GET;
//print_r($desids);exit();
genUnreleasedPDF($desids);

    
     function genUnreleasedPDF($desids){
...
//        $todays_date = date('Y-m-d');
        ...
        if(!empty($desids)){
            ...
            $itemids = implode(",", $desids);
            $date = date('Y-m-d');
//            $date = date('2024-10-23');
            $q = "select id from it_items where (createtime>='$date 00:00:00' or updatetime>='$date 00:00:00') and curr_qty>0 and design_id in($itemids)";
            $obj = $db->fetchObjectArray($q);
            $ids = array();
            foreach ($obj as $o) {
                $ids[] = $o->id;  
            }
            $remainid = implode(",", $ids);
...
//            exit();
            ...
            $parent_dir = str_replace('/ajax', '/lib/grnPDFClass/', $parent_dir);
            $dir=$parent_dir.'/pdf_files/';
//            print_r($parent_dir);exit();
//            $dir='pdf_files/';
            $date = date('YmdHis');
