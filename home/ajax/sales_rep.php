...
try {
    ...
    if ((isset($store_id) && trim($store_id) != "") && (isset($date1) && trim($date1) != "")) {
        ...
        $qrry = "select o.id as id,o.store_id as store_id,c.store_name as store_name,c.usertype as usertype,c.store_type as store_type,c.is_bhmtallyxml as is_bhmtallyxml,o.user_id as user_id,o.salesman_code as salesman_code ,o.bill_no as bill_no,o.tickettype as tickettype,o.quantity as quantity,o.amount as amount,o.discount_val as discount_val,o.discount_pct as discount_pct,o.voucher_amt as voucher_amt,o.tax as tax,o.bill_datetime as bill_datetime,o.inactive as inactive,o.ck_order_id as ck_order_id,o.total_taxable_value as total_taxable_value,o.total_tax_value as total_tax_value,o.total_cgst_value as total_cgst_value,o.total_sgst_value as total_sgst_value,o.total_igst_value as total_igst_value,o.sub_total as sub_total,o.net_total as net_total,o.cust_name as cust_name,o.cust_phone as cust_phone,c.Area as Area,c.city as city,c.Location as Location, c.status as status,if((select state from states where id=c.state_id) is null,'-', (select state from states where id=c.state_id)) as state ,if((select region from region where id=c.region_id) is null,'-', (select region from region where id=c.region_id)) as region,o.createtime as createtime,o.updatetime as updatetime from it_orders o,it_codes c where $storeClause  and o.store_id = c.id and o.is_sent =0 $dQuery";
