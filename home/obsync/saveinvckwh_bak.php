...
//                    echo "select * from it_invoices where invoice_no=$invoice_no";
                    ...
                    if(isset($exists)){                                        
                        ...
                        $sqlquery = "update it_invoices set invoice_text = $invoice_text , invoice_amt =  $invoice_amt where id = $exists->id ";
...
//                    $invoice_dt = $db->safe($invfields[2]);
                    ...
                    $query = "insert into it_invoices set invoice_text = $invoice_text,invoice_no=$invoice_no, invoice_dt=$invoice_dt, invoice_type=$invoice_type, invoice_amt=$invoice_amt, invoice_qty=$invoice_qty , total_mrp = $total_mrp , discount_1 = $discount_1 , discount_2 = $discount_2, tax = $tax , tax_type = $tax_type , tax_percent = $tax_percent , payment = $payment , no_of_challans = $noofchallans , challan_nos =  $challan_numbers $iClause ";
                    //echo "<br/>$query<br/>";
    ...
    //		$sp_code = $db->safe($fields[1]);
                        ...
                        $query = "insert into it_invoice_items set invoice_id=$invoice_id, item_code=$ck_code,  price=$price, quantity=$quantity $liClause ";                        
                        //echo "<br/>".$query."<br/>";
...
//                        if (!$inserted) { break; }
           ...
           if($invoice_type == "'0'" || $invoice_type == "'6'"){  //echo "here";
...
//           print "<br> inside if"   ;
          ...
          // echo "<br/>$query<br/>";
           ...
           //put same invoice info into it_sp_invoices to avoid depencies of that table
            $qursp="insert into it_sp_invoices set invoice_text = $invoice_text,store_id=$store_id, invoice_no=$invoice_no, invoice_dt=$invoice_dt, invoice_amt=$invoice_amt, invoice_type=$invoice_type,invoice_qty=$invoice_qty, total_mrp = $total_mrp , discount_1=$discount_1,discount_2=$discount_2, tax=$tax, payment = $payment, tax_type = $tax_type , tax_percent = $tax_percent ,ck_invoice_id=$invoice_id";
    ...
    //		$sp_code = $db->safe($fields[1]);
                        ...
                        $tot_qty += $quantity;
                        $tot_amt += ($price * $quantity);
                        $query = "insert into it_sp_invoice_items set invoice_id=$sp_id, barcode=$ck_code, price=$price, quantity=$quantity";                  
                        //echo "<br/>".$query."<br/>";
...
//                        if (!$inserted) { break; }
		...
		//$db->execInsert($query);
           ...
           if(trim($pickingId) != ""){
               ...
               if(isset($orderObj)){
                    ...
                    if($invoice_type == "'0'" || $invoice_type == "'6'"){ //stock balance only for sale type inv
        ...
        if($invoice_type == "'0'" || $invoice_type == "'6'"){
...
//            }
            if($invoice_type == "'0'" || $invoice_type == "'6'"){ // only sales inv shld get inserted in it_server_changes
                ...
                $query = "select sp.*  from  it_sp_invoices sp where sp.id = $sp_id and sp.invoice_type in ('0','6')";
