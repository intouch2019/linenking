...
$type="linequantity"; // 1=numOrders, 2=revenue
if (isset($_GET['type'])) { $type=$_GET['type']; }
if ($type=="linequantity") {
    $orderClause = " order by totqty ";
} else {
    $orderClause = " order by totval ";
}
if (isset($_GET['mrp']) && $_GET['mrp']=="1") {$mrpClause =""; }
else if (isset($_GET['mrp'])) {
    $mrpClause = " and o.MRP=".$_GET['mrp'];
} else {$mrpClause =""; }
...
$ctgs = "";
$ctgObjs = $db->fetchObjectArray("select * from it_categories ");
foreach ($ctgObjs as $cObj) {
    // 29 is the 'others' ctg id so skip it
    if ($cObj->id != 29) { $ctgs .= "$cObj->id,"; }
}
$ctgs = substr($ctgs, 0, -1);
//print $ctgs;
try {
	$db = new DBConn();
	//$query = "select o.ctg_id,o.ctg_name,sum(o.order_qty) as totqty,sum(o.MRP) as totval from it_ck_pickgroup p, it_ck_orderitems o where o.order_id in (p.order_ids) and o.ctg_id in ($ctgs) $mrpClause $d1Clause $d2Clause group by o.ctg_id $orderClause desc";
        $query = "select i.ctg_id,c.name as ctg_name ,sum(o.order_qty) as totqty,sum(o.MRP) as totval from it_ck_pickgroup p, it_ck_orderitems o , it_items i , it_categories c where o.order_id in (p.order_ids) and o.item_id = i.id and i.ctg_id = c.id and i.ctg_id in ($ctgs) $mrpClause $d1Clause $d2Clause group by i.ctg_id $orderClause desc";
//print "$query<br />";
...
foreach ($objs as $obj) {
	if ($type == "linequantity") {
        ...
        $ctg_name = "";
        for($i=0;$i<strlen($obj->ctg_name);$i++){
                $ctg_name .=$obj->ctg_name[$i];
                if($obj->ctg_name[$i]== " "){
                    $ctg_name .="\n";
                }
            }
        array_push($xlabels,$ctg_name);
	$tmp->set_on_click( "loadByCategory('".$obj->ctg_id."')" );
	$pValues[] = $tmp;
