...
//             }
        ...
        if (!$exist && !$texist && (!$storep || !$storep2)) {
            ...
            if (count($errors) == 0) {
                ...
                $qry = "insert into it_codes set  created_by=$store->id,monthlyrent='$monthlyrent',carpet='$resfloor',facade=$facad,nach_limit= $cust_nach_limit, code=$storec, password=$pass,license=md5(concat($id,'$license')), store_name=$dealer_name, store_number=$new_store_number, address=$address, state_id=$nstate, region_id=$region, city=$city, zipcode = $zipcode , owner=$name, phone=$phone, phone2=$phone2, email=$email, email2=$email2, status=$status, upi_name =$upi_name, upi_id =$upi_id, vat=$vat,gstin_no=$gstin_no, usertype=" . UserType::Dealer . " , tally_name = $tally_name ,UMRN=$umrn,cust_tobe_debited=$cust_tobe_debtd,cust_ifsc_or_mcr=$cust_ifsc_mcr,cust_debit_account=$cust_debit_account,Area=$area,Location=$location,is_natch_required=$is_natch1,sequence=$maxseq,tax_type = $taxtype $sClause $addquery ";
                // print "<br>QUERY: $qry";
