...
//       
        ...
        if ($store->usertype==UserType::Admin || $store->usertype==UserType::CKAdmin) {
            ...
            if(isset($storetype) && trim($storetype)!=""){ $sClause .= " , store_type = $storetype"; }
            
            
            if(isset($is_autorefill) && trim($is_autorefill)!=""){ $sClause .= " , is_autorefill = $is_autorefill"; }
            if(isset($sbstock_active) && trim($sbstock_active)!=""){ $sClause .= " , sbstock_active = $sbstock_active"; }  //$is_autorefill  $sbstock_active $composite_billing_opted
            if(isset($composite_billing_opted) && trim($composite_billing_opted) != "") { $sClause .= " , composite_billing_opted = $composite_billing_opted";}
