    ...
    if (!$storeid) { print "Missing parameter. Please report this error."; return; }
...
if ($old_disc != $effecteddisc) {
    ...
    if (!$store_name || !$address || !$city || !$zip || !$owner || !$phone || !$email || !$gstin_no || !$taxtype || !$nstate|| !$region || !$status) { $errors['storec'] = "Please enter value for all required field marked with *"; }
    else {
    try {
    ...
    //        $tallyname = $db->safe($tally);
            ...
            if(isset($octroi) && trim($octroi) != ""){ $discquery .= " , octroi = $octroi " ; }
...
//            if(isset($nonclaim) && trim($nonclaim) != ""){ $discquery .= " , nonclaim = $nonclaim " ; }
            ...
            $addquery = "";
            if(isset($is_closed) && trim($is_closed)!=""){ $addquery .= " , is_closed = $is_closed"; }
            //if(isset($umrn) && trim($umrn)!=""){ $addquery .= " , UMRN = $umrn"; }
            if(isset($storetype) && trim($storetype)!=""){ $addquery .= " , store_type = $storetype"; }
            if(isset($is_autorefill) && trim($is_autorefill)!=""){ $addquery .= " , is_autorefill = $is_autorefill"; }
            if(isset($sbstock_active) && trim($sbstock_active)!=""){ $addquery .= " , sbstock_active = $sbstock_active"; }
...
//                   
                    if ($password) { $query .= ",password=".$db->safe(md5($password)); }
