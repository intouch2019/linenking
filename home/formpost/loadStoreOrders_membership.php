...
function createitems($newdir) {
    ...
    foreach ($objWorksheet->getRowIterator() as $rowIndex => $row) {
        ...
        if (count($cells) >= 3) {
            ...
            if (!isset($store_items[$storecode])) {
                $store_items[$storecode] = [];
            }

            if ($qty > 0) {
                $store_items[$storecode][] = array('barcode' => $fixed_barcode, 'qty' => $qty);
            }
        }
    }

    // Save order for each store
    foreach ($store_items as $storecode => $items) {
        $return .= "<br/>" . saveOrder($storecode, $items);
    }
...
function saveOrder($store, $items) {
    ...
    $store = $db->safe($store);
    $storeinfo = $db->fetchObject("select id,inactive,is_closed,store_number,store_name,is_natch_required from it_codes where id =$store");
    if (!$storeinfo) {
...
//            $totalqty = $db->fetchObject("select i.ctg_id, i.curr_qty from it_items i, it_ck_designs d where i.barcode=$itemcode and i.ctg_id = d.ctg_id and i.design_no = d.design_no and d.active=1 and i.curr_qty > 0");
            ...
            $totalqty = $db->fetchObject("select i.ctg_id, i.curr_qty, i.design_no,i.is_design_mrp_active from it_items i, it_ck_designs d where i.barcode=$itemcode and i.ctg_id = d.ctg_id and i.design_no = d.design_no");
            if (!$totalqty) {
                continue;
            }
//            if (!$totalqty->active) {
...
//	print "$query<br />";
    ...
    return "Order $order_no placed for qty:$summary->tot_qty, amount:$summary->tot_sum. for store $storeinfo->store_name <br />$msg";
}