...
if (isset($material)) { $material = trim($material); } else $material = false;
if ($material == "") { $material = false; }
try {
	$print_ids = array(); $total = 0;
	if (!$batch_id) { $errors['batch_id'] = "Missing Batch ID"; }
...
if (!$material) $print_material = $item->material;
else $print_material = $material;
if($item->ctg_id=='24'){ //ctg_id for SHIRT PIECE
    $mtr = "/mtr";
    $fontsz = "10px";
}else{ $mtr=""; $fontsz = "11px"; }
$html .=
"<page>
<br />
