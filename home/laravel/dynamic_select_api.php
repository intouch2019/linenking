...
$query = "SELECT ic.id as maintenance_store_id ,ic.code, ic.store_name, ic.sequence, ic.tally_name, ic.accountinfo, ic.owner, ic.address, ic.city, ic.zipcode, ic.phone, ic.phone2, ic.email, ic.email2, ic.gstin_no, ic.store_number, ic.pancard_no, ic.min_stock_level, ic.max_stock_level, CASE ic.tax_type WHEN 1 THEN 'Inside Maharashtra' WHEN 2 THEN 'Outside Maharashtra' ELSE 'NA' END AS tax_type, CASE ic.is_autorefill WHEN 1 THEN 'YES' WHEN 0 THEN 'NO' ELSE 'NA' END AS is_autorefill, CASE ic.is_tallyxml WHEN 1 THEN 'YES' WHEN 0 THEN 'NO' ELSE 'NA' END AS is_tallyxml, ic.retail_saletally_name, ic.retail_sale_card_name, ic.retail_sale_cash_name, ic.retail_sale_upi_name, ic.retail_sale_bank_name, ic.cash_receipt_name, CASE ic.is_bhmtallyxml WHEN 1 THEN 'YES' WHEN 0 THEN 'NO' ELSE 'NA' END AS is_bhmtallyxml, ic.autorefil_dttm, CASE ic.is_closed WHEN 1 THEN 'YES' WHEN 0 THEN 'NO' ELSE 'NA' END AS is_closed, ic.inactivated_by, ic.inactivating_reason, ic.isastore, ic.disablelogins_reason, ic.inactive_dttm, ic.loginsdisable_by, ic.disablelogins_dttm, ic.distance, s.state AS state_id, ic.cust_tobe_debited, ic.UMRN, ic.cust_debit_account, ic.cust_ifsc_or_mcr, ic.Area, CASE ic.is_natch_required WHEN 1 THEN 'YES' WHEN 0 THEN 'NO' ELSE 'NA' END AS is_natch_required, ic.discountset, ic.Location, ic.ds_taxable_amt, ic.remark, CASE ic.composite_billing_opted WHEN 1 THEN 'YES' WHEN 0 THEN 'NO' ELSE 'NA' END AS composite_billing_opted, ic.ds_remark, CASE ic.status WHEN 1 THEN 'L to L' WHEN 2 THEN 'New L to L' WHEN 3 THEN 'New' WHEN 4 THEN 'Closed' ELSE 'NA' END AS status, r.region AS region_id, CASE ic.mask_margin WHEN 1 THEN 'YES' WHEN 0 THEN 'NO' ELSE 'NA' END AS mask_margin, ic.emandate_msgid, ic.upi_name, ic.upi_id, ic.paymentlink, ic.temp_sequence, oms.code AS omscofostoreid, ic.level, CASE ic.is_omscofo WHEN 1 THEN 'YES' WHEN 0 THEN 'NO' ELSE 'NA' END AS is_omscofo, CASE ic.is_warehouse WHEN 1 THEN 'YES' WHEN 0 THEN 'NO' ELSE 'NA' END AS is_warehouse, CASE ic.sbstock_active WHEN 1 THEN 'YES' WHEN 0 THEN 'NO' ELSE 'NA' END AS sbstock_active, ic.carpet, ic.monthlyrent, ic.facade, ic.inactive_bydatasync, ic.nach_limit, ic.saleback_starttime, ic.cust_bank_name, ic.ownership, ic.saleback_endtime, old_store.code AS old_id, ic.inactive, ic.keycheck, CASE ic.store_type WHEN 1 THEN 'NormalStore' WHEN 2 THEN 'Store50percent' WHEN 3 THEN 'CompanyStore' ELSE 'NA' END AS store_type FROM it_codes ic LEFT JOIN states s ON s.id = ic.state_id LEFT JOIN region r ON r.id = ic.region_id LEFT JOIN it_codes oms ON oms.id = ic.omscofostoreid LEFT JOIN it_codes old_store ON old_store.id = ic.old_id WHERE ic.usertype = 4";
}else if($table_name=="it_current_stock"){
$query = "SELECT ic.store_id,ic.quantity,ic.barcode FROM it_current_stock ic JOIN it_codes c ON ic.store_id = c.id WHERE c.is_closed = 0 AND c.usertype = 4 AND ic.quantity > 0 AND ic.barcode LIKE '890200%'";
