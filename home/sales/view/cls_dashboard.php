...
class cls_dashboard extends cls_renderer {
	...
	function __construct($params=null) {
		...
		if (isset($this->params['id'])) {
			$id = $this->params['id'];
			if ($this->currStore->id == 37 && $id >= 34 and $id <= 36) { 
				$storeid=$id;
				$clsCodes = new clsCodes();
				$codeInfo = $clsCodes->getCodeInfoById($id);
				$_SESSION['currStore']=$codeInfo;
				$_SESSION['corpStore']=$this->currStore;
				$this->currStore = $codeInfo;
				header("Location: ".DEF_SITEURL."store/dashboard");
			} else {
		...
		$db = new DBConn();
		$query = "select min(date(bill_datetime)) as mindate, max(date(bill_datetime)) as maxdate, sum(bill_amount) as totamount, sum(bill_quantity) as totitems, count(*) as totreceipts from it_orders where storeid=$storeid and status>0 and inactive=0";
...
<br />
<label style="width:100px;">Total Receipts:</label> <?php echo $summary->totreceipts; ?><br />
