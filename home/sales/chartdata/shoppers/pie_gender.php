...
try {
	$db = new DBConn();
	$query = "select p.sex, count(*) as num from it_orders o, it_userprofile p where o.userid=p.userid $d1Clause $d2Clause group by p.sex";
	$objs = $db->fetchObjectArray($query);
	$pValues = array();
	$pColors = array(); $count=0;
	foreach ($objs as $obj) {
		$genderName = $UserProfile["gender"][$obj->sex];
		$pValues[] = new pie_value(intval($obj->num), $genderName);
