...
try {
	$db = new DBConn();
	$query = "select p.occupation, count(*) as num from it_orders o, it_userprofile p where o.userid=p.userid $d1Clause $d2Clause group by p.occupation";
	$objs = $db->fetchObjectArray($query);
	$pValues = array();
	$pColors = array(); $count=0;
	foreach ($objs as $obj) {
		$pieText = $UserProfile["occupation"][$obj->occupation];
