...
try {
	$db = new DBConn();
	$query = "select p.ismarried, count(*) as num from it_orders o, it_userprofile p where o.userid=p.userid $d1Clause $d2Clause group by p.ismarried";
	$objs = $db->fetchObjectArray($query);
	$pValues = array();
	$pColors = array(); $count=2;
	foreach ($objs as $obj) {
		$pieText = $obj->ismarried == "0" ? "Single" : "Married";
