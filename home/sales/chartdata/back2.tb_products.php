...
$d2Clause = " and date(bill_datetime) <= '".$d2."' ";
	 ...
	 */
	$sQuery = "
		select SQL_CALC_FOUND_ROWS r.itemname, sum(rl.linequantity) as unitssold, round(sum(rl.linetotal),0) as totalamount, '-' as openingstock, '-' as stockin, '-' as closingstock, r.id from it_rawitems r, it_rawitemlines rl, it_orders o
		$sWhere
		$sHaving
		$sOrder
		$sLimit
...
//	$logger->logInfo($sQuery);
	...
	if (count($objs) > 0) {
		$itemids = array();
		foreach ($objs as $obj) { $itemids[] = $obj->id; }
		$itemids = join(",", $itemids);
		$objs2 = $db->fetchObjectArray("
			select r.id, 
			(cs1.stock_in - cs1.sales) as openingstock, 
			(cs2.stock_in - cs1.stock_in) as stockin, 
			(cs2.stock_in - cs2.sales) as closingstock 
			from it_rawitems r, it_closing_stock cs1, it_closing_stock cs2
			where r.id in ($itemids)
			and r.id = cs1.rawitemid and cs1.closing_dt = '$d0' 
			and r.id = cs2.rawitemid and cs2.closing_dt = '$d2' 
		");
		foreach ($objs2 as $obj2) {
			$closing_stock[$obj2->id] = array($obj2->openingstock, $obj2->stockin, $obj2->closingstock);
		}
	}
	...
	foreach ($objs as $obj)
	{
		...
		{
			if ( $aColumns[$i] == "openingstock" && isset($closing_stock[$obj->id])) {
				$row[] = $closing_stock[$obj->id][0];
			} else
			if ( $aColumns[$i] == "stockin" && isset($closing_stock[$obj->id])) {
				$row[] = $closing_stock[$obj->id][1];
			} else
			if ( $aColumns[$i] == "closingstock" && isset($closing_stock[$obj->id])) {
				$row[] = $closing_stock[$obj->id][2];
			} else
			if ( $aColumns[$i] != ' ' )
			{
				/* General output */
