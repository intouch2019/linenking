...
//        print "\n$iquery"; exit();
    ...
    //<------------       Discount_scheme::loyalty_membership data insertion ends        ------------>
      ...
      $eossquery = "SELECT x.store_id, x.store_name, x.discountset, CONCAT(x.tax_rate_mrp,'-',x.tax_rate_value) AS tax_combo, SUM(CASE WHEN x.has_discount = 1 THEN x.final_mrp ELSE 0 END) AS total_mrp, SUM(x.discount_val) AS total_discount, SUM(CASE WHEN x.has_discount = 1 THEN x.final_value ELSE 0 END) AS totalvalue FROM ( SELECT c.id AS store_id, c.store_name, c.discountset, CASE WHEN o.discount_pct IS NOT NULL OR IFNULL(oi.discount_val,0) > 0 THEN 1 ELSE 0 END AS has_discount, CASE WHEN o.tickettype = 1 THEN ABS(oi.quantity) ELSE oi.quantity END AS qty, CASE WHEN o.discount_pct IS NOT NULL THEN (((100 - o.discount_pct)/100) * oi.price) * CASE WHEN o.tickettype = 1 THEN ABS(oi.quantity) ELSE oi.quantity END ELSE oi.price * CASE WHEN o.tickettype = 1 THEN ABS(oi.quantity) ELSE oi.quantity END END AS final_value, i.MRP AS final_mrp, IFNULL(oi.discount_val,0) AS discount_val, CASE WHEN ict.tax_rate IS NOT NULL THEN ict.tax_rate * 100 WHEN ( CASE WHEN o.discount_pct IS NOT NULL THEN ((100 - o.discount_pct)/100) * i.MRP ELSE i.MRP END ) > 2625 THEN 18 ELSE 5 END AS tax_rate_mrp, CASE WHEN ict.tax_rate IS NOT NULL THEN ict.tax_rate * 100 WHEN ( CASE WHEN o.discount_pct IS NOT NULL THEN (((100 - o.discount_pct)/100) * oi.price) * CASE WHEN o.tickettype = 1 THEN ABS(oi.quantity) ELSE oi.quantity END ELSE oi.price * CASE WHEN o.tickettype = 1 THEN ABS(oi.quantity) ELSE oi.quantity END END ) > 2625 THEN 18 ELSE 5 END AS tax_rate_value FROM it_orders o JOIN it_order_items oi ON oi.order_id = o.id JOIN it_items i ON i.id = oi.item_id JOIN it_codes c ON o.store_id = c.id JOIN states s ON s.id = c.state_id JOIN region r ON c.region_id = r.id LEFT JOIN it_category_taxes ict ON ict.category_id = i.ctg_id JOIN ( SELECT o.id AS order_id, MAX(CASE WHEN oi.quantity < 0 THEN 1 ELSE 0 END) AS has_negative_qty, MAX(CASE WHEN IFNULL(oi.discount_val,0) > 0 THEN 1 ELSE 0 END) AS has_discount FROM it_orders o JOIN it_order_items oi ON oi.order_id = o.id JOIN it_order_payments p ON p.order_id = o.id AND TRIM(p.payment_name) != 'Loyalty' WHERE o.tickettype = 0 AND o.store_id IN ($store_id_str) AND o.bill_datetime BETWEEN '$from_dt' AND '$to_dt' GROUP BY o.id ) bt ON bt.order_id = o.id WHERE o.store_id IN ($store_id_str) AND o.bill_datetime BETWEEN '$from_dt' AND '$to_dt' ) AS x GROUP BY x.store_id, tax_combo ORDER BY x.store_id, tax_combo;";
